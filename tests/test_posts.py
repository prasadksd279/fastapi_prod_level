...
def test_get_all_posts(authorized_client, test_posts):
    ...
    # validate = [schemas.PostOut.model_validate(p) for p in res.json()]
    # print(validate)
...
def test_get_one_post(authorized_client, test_posts):
    ...
    # print(res.json())
    # post = schemas.PostOut.model_validate(res.json())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("awsome new title1", "awsome new content1", False),
        ("awsome new title2", "awsome new content2", True),
    ],
)
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post(
        "/posts/", json={"title": "some_title", "content": "content"}
    )
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "some_title"
    assert created_post.content == "content"
    assert created_post.owner_id == test_user["id"]


def test_unauthorized_user_create_posts(client, test_user, test_posts):
    res = client.post("/posts/", json={"title": "some_title", "content": "content"})
    assert res.status_code == 401
...
def test_update_post(authorized_client, test_user, test_posts):
    ...
    updated_post = schemas.Post.model_validate(res.json())
    assert res.status_code == 200
...
def test_unauthorized_user_delete_poost(client, test_user, test_posts):
